...
#include "AI/Tasks/BTTask_OrderMoveTo.h"
...
UBTTask_OrderMoveTo::UBTTask_OrderMoveTo(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	NodeName = "Move To";
	...
	BlackboardKey.AddObjectFilter(this, GET_MEMBER_NAME_CHECKED(UBTTask_OrderMoveTo, BlackboardKey), AActor::StaticClass());
	BlackboardKey.AddVectorFilter(this, GET_MEMBER_NAME_CHECKED(UBTTask_OrderMoveTo, BlackboardKey));
}

EBTNodeResult::Type UBTTask_OrderMoveTo::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (NodeResult == EBTNodeResult::InProgress && bObserveBlackboardValue)
	{
		...
		if (ensure(BlackboardComp))
		{
			...
			{
				UE_VLOG(MyController, LogBehaviorTree, Warning, TEXT("UBTTask_OrderMoveTo::ExecuteTask \'%s\' Old BBObserverDelegateHandle is still valid! Removing old Observer."), *GetNodeName());
				BlackboardComp->UnregisterObserver(BlackboardKey.GetSelectedKeyID(), MyMemory->BBObserverDelegateHandle);
			}
			MyMemory->BBObserverDelegateHandle = BlackboardComp->RegisterObserver(BlackboardKey.GetSelectedKeyID(), this, FOnBlackboardChangeNotification::CreateUObject(this, &UBTTask_OrderMoveTo::OnBlackboardValueChange));
		}
	}	
...
EBTNodeResult::Type UBTTask_OrderMoveTo::PerformMoveTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (MyController && MyBlackboard)
	{
		...
		if (BlackboardKey.SelectedKeyType == UBlackboardKeyType_Object::StaticClass())
		{
			...
			{
				UE_VLOG(MyController, LogBehaviorTree, Warning, TEXT("UBTTask_OrderMoveTo::ExecuteTask tried to go to actor while BB %s entry was empty"), *BlackboardKey.SelectedKeyName.ToString());
			}
		}
		...
		if (MoveReq.IsValid())
		{
			...
			else
			{
				...
				FPathFollowingRequestResult RequestResult = MyController->MoveTo(MoveReq, &Path);
...
UAITask_MoveTo* UBTTask_OrderMoveTo::PrepareMoveTask(UBehaviorTreeComponent& OwnerComp, UAITask_MoveTo* ExistingTask, FAIMoveRequest& MoveRequest)
{
...
EBlackboardNotificationResult UBTTask_OrderMoveTo::OnBlackboardValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
EBTNodeResult::Type UBTTask_OrderMoveTo::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	FBTMoveToTaskMemory* MyMemory = CastInstanceNodeMemory<FBTMoveToTaskMemory>(NodeMemory);
	if (!MyMemory->bWaitingForPath)
	{
...
void UBTTask_OrderMoveTo::OnTaskFinished(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTNodeResult::Type TaskResult)
{
	FBTMoveToTaskMemory* MyMemory = CastInstanceNodeMemory<FBTMoveToTaskMemory>(NodeMemory);
	MyMemory->Task.Reset();
...
void UBTTask_OrderMoveTo::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
...
void UBTTask_OrderMoveTo::OnMessage(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, FName Message, int32 SenderID, bool bSuccess)
{
...
void UBTTask_OrderMoveTo::OnGameplayTaskDeactivated(UGameplayTask& Task)
{
...
FString UBTTask_OrderMoveTo::GetStaticDescription() const
{
...
void UBTTask_OrderMoveTo::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
uint16 UBTTask_OrderMoveTo::GetInstanceMemorySize() const
{
	return sizeof(FBTMoveToTaskMemory);
}

void UBTTask_OrderMoveTo::PostLoad()
{
	Super::PostLoad();
...
FName UBTTask_OrderMoveTo::GetNodeIconName() const
{
	return FName("BTEditor.Graph.BTNode.Task.MoveTo.Icon");
}

void UBTTask_OrderMoveTo::OnNodeCreated()
{
	bStopOnOverlapNeedsUpdate = false;
}
